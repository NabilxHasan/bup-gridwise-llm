...
def main():
    ...
    parser.add_argument("--url", default="http://localhost:8000", help="Base URL of service")
    ...
    t0 = time.time()
    try:
        h_resp = requests.get(f"{base_url}/health", timeout=10)
        h_time = time.time() - t0
    ...
    for c in data["cases"]:
        ...
        t_start = time.time()
        resp = requests.post(f"{base_url}/optimize-energy", json=inp, timeout=30)
    ...
    if all_ok and p95_lat <= 5.0:
        print("ALL TESTS PASSED & QUALIFIED FOR FULL RELIABILITY/PERFORMANCE POINTS!")
